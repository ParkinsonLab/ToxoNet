	  ...
	  //printf("%d,%d\n",rows,columns); 
          ...
          for(p=0;p<rows-1;p++)   //First coelution profile
          {
				...
				for(i=0;i<2000;i++)
				{
					...
					tmp_matrix[p][n]=(float)tmp_matrix[p][n]/column_totals[n];	//normalizing peptides to frequencies
					//tmp_matrix[p][n]=(float)matrix[p][n]/row_totals[p];
					//tmp_matrix[p][n]=matrix[p][n];


					tmp_matrix[q][n]=matrix[q][n]+(1.0/columns);
					tmp_matrix[q][n]=gsl_ran_poisson(r,tmp_matrix[q][n]);
					tmp_matrix[q][n]=(float)tmp_matrix[q][n]/column_totals[n];
