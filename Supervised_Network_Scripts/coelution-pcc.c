...
//This code calculates pearson correlation coefficient (pcc) for a elution matrix
...
//Usage:
//Eg: ./coelution-pcc  matrix_curated_g2e2ma > output.csv
//Compile as follows
//gcc -o coelution-pcc coelution-pcc.c -lgsl -lgslcblas -lm
                ...
                for(q=p+1;q<rows;q++)   //Second coelution profile
                {
			        p_mean=0.0;
	                        q_mean=0.0;
                                ...
                                for(n=0;n<columns;n++)
                                {
                                        p_mean=p_mean+matrix[p][n];
                                        q_mean=q_mean+matrix[q][n];
                                }
                                ...
                                for(n=0;n<columns;n++)
                                {
                                        p_single=matrix[p][n]-p_mean;
                                        p_square=p_single*p_single;

                                        q_single=matrix[q][n]-q_mean;
