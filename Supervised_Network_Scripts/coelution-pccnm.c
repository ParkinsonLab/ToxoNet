...
//This code generates pearson correlation coefficients between two spectral profiles - after adding noise to the profiles - 1000 times - and takes the average of the PCCs..
...
To compile on scinet..
module load intel/17.0.0
module load gsl
module load gcc
gcc -o coelution_pccnm coelution_pccnm.c -lgsl -lgslcblas -lm -Ofast
          ...
          for(p=0;p<rows-1;p++)   //First coelution profile
          {
                for(q=p+1;q<rows;q++)   //Second coelution profile
                {
			        p_mean=0.0;
                        ...
                        //1000 random_poissonizations for the two coelution profiles
                        for(x=0;x<1000;x++)
                        {
                                pcc_poisson[x]=0.0;
				...
				//printf("%d\n",x);
                                ...
                                //Get poisson modelled values for all data points in first and second coelution profiles, add a constant value to the value (1/number of columns) - normalize them by their column-wise totals and their get their mean
				...
				for(i=0;i<2000;i++)
				{
					...
					tmp_matrix[p][n]=matrix[p][n]+(1.0/columns);	//Modelling the peptide count in each fraction as a poisson process
					tmp_matrix[p][n]=gsl_ran_poisson(r,tmp_matrix[p][n]);	//noise term added to the maximum likelihood estimate of each fraction
					//tmp_matrix[p][n]=(float)matrix[p][n]/column_totals[n];	//normalizing peptides to frequencies
					...
					//tmp_matrix[q][n]=(float)matrix[q][n]/column_totals[n];
				...
				pcc_poisson[x]=numerator/denominator;
