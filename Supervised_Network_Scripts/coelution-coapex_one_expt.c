...
//2. A fraction of how many of the fractions with modal abundance are matched in the two proteins - for eg. if 2 fractions in protein A have modal abundance and 3 fractions in protein B have modal abundance, and only 2 match, coapex score would be 2(2)/2+3 - this would be coapex score 3
...
//Column3 - Coapex score type2 (averaged score - the higher of the two scores.. since the denominator can vary for protein1 vs. protein2)
...
int main(int argc, char **argv)
{
  ...
  char codes[5000][500];
  int fractions[5000][200];

  int num_prot=0,ct=0,ct1=0,ct2=0;
  int i=0,j=0,x=0,y=0,a=0,b=0;

  int match_fractions=0,modal_value1=0,modal_value2=0,one_modal_fractions[200],two_modal_fractions[200];
  int coapex_score1=0;
  ...
  while(line[0]!='Z')
  {
	...
	for(i++;line[i]!='\0';i++)
	{
		...
		{
			tmp[j]='\0';
			fractions[num_prot][ct]=atoi(tmp);
  ...
  for(i=0;i<num_prot-1;i++)
  {
	...
	for(j=i+1;j<num_prot;j++)
	{
		...
		printf("%s\t%s\t%d\t%.3f\t%d\n",codes[i],codes[j],coapex_score1,coapex_score2,match_fractions);	
	}
