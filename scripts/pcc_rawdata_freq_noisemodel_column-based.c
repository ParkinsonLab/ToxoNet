...
int main(int argc, char **argv)
{
  ...
  float data1[2000],data2[2000],data1p[2000],data2p[2000],pcc_poisson[2000],matrix1[2000][500];
  ...
  int i=0,j=0,p=0,q=0,z=0,columns=0,rows=0,x=0,m=0,n=0;

  char line1[50000],line2[50000],tmpval[30],outfile[500],tmpstr[500],codes[100000][500];
...
//Extracting details from file 1
  ...
  //printf("Here\n");
  //Skip first line as it contains headers - just derive number of columns from that (-1 for the codes)
  parseline(line1,fp1);
  columns=0;
  for(i=0;line1[i]!='\0';i++)
  {
  ...
  //Getting all the gene codes from column1
	rows=0;
  while(line1[0]!='Z')
  {
	//printf("%s\n",line1);
	...
	for(i++;line1[i]!='\0';i++)
        {
                ...
                matrix1[m][n]=atof(tmpval);
                printf("%s,%f ",tmpval,matrix1[m][n]);
                n++;
        }
