...
awk '$10>0 {print $1","$2}' $1 | grep -v "protein1" > tmp_string
...
do
	...
	score=`grep "$c1" $1 | grep "$c2" | awk '{print $10}' | sort -u`
	...
	then
		for code1 in `grep -Fw "$c1" $2 | awk -F "\t" '{print $1"#"$5}' | sed 's/ /~/g'`
		do
			for code2 in `grep -Fw "$c2" $2 | awk -F "\t" '{print $1"#"$5}' | sed 's/ /~/g'`
			do
