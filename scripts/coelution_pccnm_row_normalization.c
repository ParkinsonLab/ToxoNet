	  ...
	  //printf("%d,%d\n",rows,columns); 
          ...
          for(p=0;p<rows-1;p++)   //First protein's coelution profile
          {
				...
				for(i=0;i<2000;i++)
				{
					...
					tmp_matrix[p][n]=gsl_ran_poisson(r,matrix[p][n]); //Modelling the peptide count in each fraction as a poisson process
					//printf("Random: %d: %.2f, %.2f\n",n,matrix[p][n],tmp_matrix[p][n]);
